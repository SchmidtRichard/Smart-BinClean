...
//If there are no results from the above query, it will continue into creating a new user
if ($result->num_rows == 0)
{
    //We're checking if all those fields has been completed on the html form
    if ($name && $username && $password && $password1 && $email && $admin_rights)
    {
        ...
        if (preg_match('/^[0-9A-Za-z\.-_]+@[0-9A-Za-z-]+\.[A-Za-z]{2,4}$/', $email))
        {
            ...
            if ($password == $password1)
            {
                ...
                account_created(); //Calls the function to pop-out the information that a new user has been created succesfully
            }
            else
            {
?>
...
?>
						<script>
						      alert("The email is not completed well! Please double check it!");
						</script>
                        <?php
            redirect();
        }
